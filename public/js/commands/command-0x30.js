/**
 * Command 0x30 - Get Non-Adaptive ANC Level
 * Handles non-adaptive ANC level queries and responses
 */
class Command30 extends BaseCommand {
    ...
    render(container) {
        console.log('Command30 render called with container:', container);
        ...
        if (!this.config || !this.config.fields || !Array.isArray(this.config.fields)) {
            console.error('Invalid config in Command30 render, using defaults');
            this.config = this.getDefaultConfig();
        }

        // Generate dynamic fields based on configuration
        const fieldsHtml = this.config.fields.map(field => {
            const fieldId = `field-${field.id}-0x30`;
            const groupId = `field-group-${field.id}-0x30`;
    ...
    setDefaultValues() {
        ...
        const packetTypeField = document.getElementById('field-packetType-0x30');
        if (packetTypeField) {
    ...
    updateFieldVisibility() {
        this.config.fields.forEach(field => {
            if (field.showWhen) {
                const triggerFieldId = `field-${field.showWhen.fieldId}-0x30`;
                const targetGroupId = `field-group-${field.id}-0x30`;
    ...
    getPayload() {
        ...
        const ancLevelElement = document.getElementById('field-ancLevel-0x30');
    ...
    getPacketType() {
        const packetTypeElement = document.getElementById('field-packetType-0x30');
