/**
 * Command 0x5F - Set Prompt Sound State
 * Handles setting prompt sound state
 */
class Command5F extends BaseCommand {
            ...
            fields: [
                {
                    ...
                    name: 'Packet Type',
                    options: [
                        { value: '0', label: 'COMMAND (set state)' },
    ...
    render(container) {
        ...
        const fieldsHtml = this.config.fields.map(field => {
            const fieldId = `field-${field.id}-0x5F`;
            const groupId = `field-group-${field.id}-0x5F`;
            let initialStyle = field.showWhen ? 'style="display: none;"' : '';
            let fieldName = field.name;

            if (isZh) {
                if (fieldName === 'Prompt Sound Enabled') fieldName = '提示音启用';
        ...
        setTimeout(() => {
            const packetTypeField = document.getElementById('field-packetType-0x5F');
            if (packetTypeField) {
    ...
    updateFieldVisibility() {
        this.config.fields.forEach(field => {
            if (field.showWhen) {
                const triggerElement = document.getElementById(`field-${field.showWhen.fieldId}-0x5F`);
                const targetGroup = document.getElementById(`field-group-${field.id}-0x5F`);
    ...
    attachListeners() {
        this.config.fields.forEach(field => {
            ...
            this.addListener(`field-${field.id}-0x5F`, field.type === 'checkbox' ? 'change' : 'change');
        });
    }
    ...
    getPayload() {
        ...
        if (packetType === 0) {
            const enabledElement = document.getElementById('field-promptSoundEnabled-0x5F');
            return enabledElement ? [enabledElement.checked ? 0x01 : 0x00] : [];
        } else {
            const statusElement = document.getElementById('field-executionStatus-0x5F');
            return statusElement ? [parseInt(statusElement.value, 16)] : [];
        }
    }

    getPacketType() {
        const element = document.getElementById('field-packetType-0x5F');
        return element ? parseInt(element.value, 10) : 0;
    }
}

// Register the command class globally
window.Command5F = Command5F;